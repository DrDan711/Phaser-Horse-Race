...
var horses;     //my group of racing horses
...
gameScene.create = function() {
  ...
  horses = this.add.group({
    key: 'horseWhiteSprite',    //this is the initial sprite, but I will overwrite this below
  ...
  // animations, need to create a run walk and stopped for each color horse (spritesheet)
  ...
  //give each horse a color param value - can use the name to select the desired animation / spritesheet
  ...
  horses.children.iterate(function (horse) {
    ...
    //assign each horse a random walking and running speed
    horse.walkingSpeed = Math.random() * 1 + 0.1;
    horse.runningSpeed = horse.walkingSpeed + Math.random() * 1 + 0.1;  
    //I make sure the running speed is faster than the walking speed by adding something to it.
